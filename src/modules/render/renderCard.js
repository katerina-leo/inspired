...
export const renderCard = (data) => {
  ...
  card.textContent = '';
  if (!data) return;
  const {id, title, description, colors, pic, price, size} = data;
          ...
          createElement('p',
            {
              ...
              textContent: size.value ? color.value ? count.value ? 'Что-то пошло не так' : 'Выберите количество' : 'Выберите цвет' : 'Выберите размер',
            },
            {
              parent: form,
              cb(p)  {
